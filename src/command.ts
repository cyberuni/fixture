...
import { type PartialPick, unpartial } from 'type-plus'
...
export async function execCommand(
	input: Pick<BaselineHandlerContext, 'casePath' | 'caseName' | 'caseType'>
): Promise<execCommand.Result>
...
function execCommandDirectly({ casePath, command, args }: execCommand.CommandInfo) {
	return execa(command, adjustArgs(args), { cwd: casePath, cleanup: true, shell: true }).then(({ stderr, stdout }) => ({
		stderr,
		stdout
	}))
}
function readCommandInfo({
	caseName,
	caseType,
	casePath
}: Pick<BaselineHandlerContext, 'caseType' | 'caseName' | 'casePath'>) {
...
function findCommandFileInfo({ caseType, casePath }: Pick<BaselineHandlerContext, 'caseType' | 'casePath'>) {
	return caseType === 'file' ? findCommandFileInfoForFile(casePath) : findCommandFileInfoForFolder(casePath)
}
