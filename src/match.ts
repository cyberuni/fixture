...
import type { DiffFormatOptions } from './diff.js'
...
export function createMatchFunction(baselinePath: string, resultPath: string, options: DiffFormatOptions) {
	...
	return function match(target = ''): Promise<any> {
		...
		return compare(match, path.join(baselinePath, target), resultTarget, options)
	}
}
...
function fillFileOrDirectory(files: { filePath: string; ctime: number }[], entryPath: string) {
	...
	if (stat.isDirectory()) {
		for (const entry of fs.readdirSync(entryPath)) fillFileOrDirectory(files, path.join(entryPath, entry))
	}
}
function removeUnchangedFiles(
	before: { filePath: string; ctime: number }[],
	after: { filePath: string; ctime: number }[]
) {
	after.forEach((v) => {
...
async function compare(match: any, baselinePath: string, resultPath: string, options: DiffFormatOptions) {
	...
	try {
		...
		context.log.debug(`comparing ${baselinePath} and ${resultPath} took ${Date.now() - time} (ms)`)
...
function getMissingBaselineMismatch(
	match: any,
	_missingFilePath: string,
	resultPath: string,
...
function getMissingResultMismatch(
	match: any,
