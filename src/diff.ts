...
import { DiffMatch, type DiffResult } from './DiffMatch.js'
...
export function formatDiff(diff: { singleLine: boolean; diff: DiffResult[] }, options: DiffFormatOptions) {
	return diff.singleLine ? formatWordsDiff(diff.diff) : formatLinesDiff(diff.diff, options)
}

function formatWordsDiff(diff: DiffResult[]) {
	return diff
		.map((part) => {
			if (part.added) return chalk.red(part.value)
			if (part.removed) return chalk.green(part.value)
...
function formatLinesDiff(diff: DiffResult[], options: DiffFormatOptions) {
	...
	const lines = prependLegend(
		lineCount > options.largeFileThreshold
			? formatManyLinesDiff(diff, lineCount, options.largeFileAmbientLines)
			: formatFewLinesDiff(diff)
	)
...
function formatManyLinesDiff(diff: DiffResult[], totalLineCount: number, numOfAmbientLines: number) {
	...
	return diffLines.map((part) => {
		...
		if (part.count) return `${padLeft(part.count, padding)}:   ${part.value}`
...
function getTrimmedLineDiffs(diff: DiffResult[], numOfAmbientLines: number) {
	...
	allLines.forEach((line) => {
		if (inRange(anchors, numOfAmbientLines, line)) {
			...
			if (line.count) lastCount = line.count
		}
	})
	return trimmedLines
}
