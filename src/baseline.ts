...
export const baseline = Object.assign(
	...
	},
	{
		...
		skip(_basePathOrOptions: string | Partial<BaselineOptions>, _handler: BaselineHandler): void {}
	}
)
...
function getShouldIncludePredicate(filter: string | RegExp | undefined) {
	if (filter instanceof RegExp) return (path: string) => filter.test(path)
	if (typeof filter === 'string') return (path: string) => minimatch(path, filter)
