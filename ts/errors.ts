...
import type { DiffResult } from './DiffMatch.js'
import { createDiff, type DiffFormatOptions, formatDiff } from './diff.js'
...
export class ExtraResultFile {
	...
	constructor(
		...
		const createDiffTime = Date.now()
		context.log.debug(`create diff for ${filePath} took ${createDiffTime - time} ms`)
		this.diff = diff.diff
		this.formattedDiff = formatDiff(diff, options)
		context.log.debug(`format diff for ${filePath} took ${Date.now() - createDiffTime} ms`)
	}
	tersify() {
...
export class Mismatch extends IsoError {
	constructor(
		public mismatches: Array<Tersible<Record<string, any>>>,
		options?: IsoError.Options
	) {
