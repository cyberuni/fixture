...
test('provided match() rejects with files in folder not match by content', () => {
	return assertron.throws(
		...
		(err) => {
			if (!(err instanceof Mismatch) || err.mismatches.length !== 1) return false

			const mismatch = err.mismatches[0]

			return (
				mismatch instanceof MismatchFile &&
...
test('provided match() rejects when missing baseline folder', () => {
	...
	return assertron.throws(
		...
		(err) =>
			err instanceof ExtraResultFile && pathEqual(err.filePath, 'fixtures/dir-miss-baseline-folder/results/case-1')
	)
})
...
test('provided match() rejectts when missing result file', () => {
	return assertron.throws(
		...
		(err) => {
			...
			const m1 = err.mismatches[0]
			return (
				m1 instanceof MissingResultFile &&
...
function pathsEqual(actuals: string[], expects: string[]) {
	actuals.forEach((a, i) => {
		assertron.pathEqual(a, expects[i])
	})
}
